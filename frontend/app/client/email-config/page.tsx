...
const fieldFocus = "focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/15";
...
function GmailCard() {
  ...
  async function sendTest() {
    ...
    const ok = await confirm({
      ...
      message: (
        <>
          A real test email will be sent to <b>{testTo}</b> through your Gmail account. Continue?
        </>
      ),
  ...
  async function save() {
    ...
    try {
      ...
      setPassword("");
      setTestResult(null);
      toast.success("Email settings saved.", { title: "Saved" });
    } catch (e) {
    ...
    <>
      <Card className="lg:col-span-2">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-500">Read your mail with a Google App Password.</p>
          </div>
        ...
        <div className="space-y-4">
          ...
          <label className="block">
            ...
            <input
              ...
              className={`w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 placeholder:text-slate-400 ${fieldFocus}`}
          ...
          <button type="button" onClick={() => setShowAdvanced((v) => !v)} className="text-sm font-medium text-emerald-600 hover:text-emerald-700">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-1">
            ...
            <button onClick={test} disabled={testing} className={btnGhost}>{testing ? "Testing…" : "Test connection (read)"}</button>
          </div>

          {/* Outgoing email test — confirms Gmail SMTP works before relying on it */}
          <div className="mt-2 rounded-xl border border-slate-200 bg-slate-50/60 p-4">
            ...
            <p className="mt-0.5 text-xs text-slate-500">Verifies outgoing mail (Gmail SMTP). You&apos;ll be asked to confirm before a real email is sent.</p>
            <div className="mt-3 flex flex-wrap items-end gap-2">
              <label className="flex-1">
                ...
                <input
                  ...
                  className={`w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 ${fieldFocus}`}
                />
              </label>
            ...
            <p className="mt-2 text-xs text-slate-400">Save your Gmail address &amp; App Password first.</p>
      ...
      <Card>
        ...
        <ol className="mt-3 space-y-3 text-sm text-slate-600">
          <li className="flex gap-2.5"><span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-600">1</span>Turn on <b>2-Step Verification</b> for your Google account.</li>
          <li className="flex gap-2.5"><span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-600">2</span>Go to <b>Security → App passwords</b> and generate one for “Mail”.</li>
          <li className="flex gap-2.5"><span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-600">3</span>Paste the Gmail address and the 16-character password here, then <b>Save</b>.</li>
        </ol>
        <a href="https://myaccount.google.com/apppasswords" target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-emerald-600 hover:text-emerald-700">
        ...
        <p className="mt-4 rounded-lg bg-slate-50 px-3 py-2.5 text-xs text-slate-500">
          Credentials are stored against your account in the database. The password is never shown again after saving.
        </p>
      </Card>
    </>
  );
  ...
  return (
    <>
      <Card className="lg:col-span-2">
        ...
        <div className="space-y-4">
          <Field label="Calendar ID" value={calendarId} onChange={setCalendarId} placeholder="you@gmail.com  or  abc123@group.calendar.google.com" />
          ...
          <label className="block">
            ...
            <textarea
              ...
              className={`w-full rounded-lg border border-slate-300 px-3 py-2 font-mono text-xs text-slate-900 placeholder:text-slate-400 ${fieldFocus}`}
              spellCheck={false}
            />
            <span className="mt-1 block text-xs text-slate-400">
              The JSON key you downloaded for the service account. Stored in your database and never shown again.
            </span>
          </label>
          ...
          <div className="flex flex-wrap items-center gap-3 pt-1">
            <button onClick={save} disabled={saving} className={btnPrimary}>{saving ? "Saving…" : "Save settings"}</button>
            <button onClick={test} disabled={testing} className={btnGhost}>{testing ? "Testing…" : "Test connection"}</button>
          </div>
        </div>
      </Card>
      ...
      <Card>
        <h3 className="font-semibold text-slate-900">How to connect</h3>
        <ol className="mt-3 space-y-3 text-sm text-slate-600">
          <li className="flex gap-2.5"><span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-600">1</span>In <b>Google Cloud Console</b>, create a project and enable the <b>Calendar API</b>.</li>
          <li className="flex gap-2.5"><span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-600">2</span>Create a <b>service account</b> and download its <b>JSON key</b>.</li>
          <li className="flex gap-2.5"><span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-600">3</span>In Google Calendar, <b>share</b> your calendar with the service account email (Make changes to events).</li>
          <li className="flex gap-2.5"><span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-600">4</span>Paste the JSON key and the calendar ID here, then <b>Save</b>.</li>
        </ol>
        <a href="https://console.cloud.google.com/apis/library/calendar-json.googleapis.com" target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-emerald-600 hover:text-emerald-700">
...
export default function EmailConfigPage() {
  return (
    <>
      <PageHeader title="Email & Calendar" subtitle="Connect your Gmail inbox and Google Calendar to this workspace." />
