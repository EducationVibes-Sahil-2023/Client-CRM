...
export const MAIN_NAV: NavItem[] = [
  ...
  { key: "calls", href: "/client/calls", label: "Call Tracking", icon: "calls", feature: "leads", module: "leads" },
  { key: "followups", href: "/client/followups", label: "Follow Up Tracker", icon: "followups", feature: "leads", module: "leads" },
...
const setupNav: NavItem[] = [
  { key: "billing", href: "/client/billing", label: "Billing", icon: "billing", adminOnly: true },
...
export default function ClientSidebar() {
  const { collapsed, mobileOpen, setMobileOpen, user, logout, hasFeature, branding, can, isAdmin } = useClient();
  ...
  const Item = ({ item }: { item: NavItem }) => {
    ...
    return (
      ...
      >
        ...
        <svg className={`h-5 w-5 flex-shrink-0 ${iconCls}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d={icons[item.icon]} strokeLinecap="round" strokeLinejoin="round" /></svg>
    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex flex-col border-r border-slate-200 bg-white transition-all duration-300 ${collapsed ? "w-20" : "w-64"} ${mobileOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
        <div className="flex h-16 items-center gap-3 px-5">
          {logoSrc ? (
            <img src={logoSrc} alt={branding.app_name} className="h-10 w-10 flex-shrink-0 rounded-xl object-cover" />
          ) : (
        ...
        <div className="border-t border-slate-100 p-3">
          <div className={`flex items-center gap-3 px-2 py-2 ${collapsed ? "justify-center" : ""}`}>
            ...
            {!collapsed && <div className="min-w-0 leading-tight"><div className="truncate text-sm font-semibold text-slate-800">{user.name || "Client Admin"}</div><div className="truncate text-xs text-slate-400">{user.email}</div></div>}
          </div>
